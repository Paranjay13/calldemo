...
public class TService extends Service {
    ...
    private void startRecording() {
        File sampleDir = new File(Environment.getExternalStorageDirectory(), "/CallLogCustom");
        if (!sampleDir.exists()) {
            sampleDir.mkdirs();
        }
            ...
            switch (state) {
                ...
                case TelephonyManager.CALL_STATE_IDLE:
                    ...
                    if (lastState == TelephonyManager.CALL_STATE_RINGING) {
                        ...
                        CallLogCustom callLogCustom=new CallLogCustom();
                        callLogCustom.setName(savedNumber);
                        callLogCustom.setTimestamp(callStartTime.getTime());
                        callLogCustom.setDuration(new Date().getTime());
                        callLogCustom.setFilePath(audiofile.getAbsolutePath());
                        callLogCustom.setId(savedNumber+audiofile.getName());
                        callLogCustom.setState("in");
                        new DBHelper(getApplicationContext()).addEntry(callLogCustom);
                    } else {
                        stopRecording();
